...
from nexussim.scene.scene_simulation import (
...
def main():
    ...
    for node_id, data in report["sim_data"].items():
        if data:
            ...
            alt.Chart(data).mark_area(color="lightblue", interpolate="step-after", line=True).encode(
                x="timestamp", y="cpu_used"
            ).save(f"total_usage_{node_id}.html")
