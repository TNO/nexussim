...
class Program(Entity):
    ...
    def execute(self, context):
        ...
        self._on_finish_handler = self._wait_once(
            EventHandler(lambda h_report: self.default_on_finish_callback(h_report, context)),
            entity=self.body,
...
def constant_load_segment(
    ...
    Note: we expect either a duration or a number of cycles but not both
    ...
    Args:
        ...
        message (dict): Represents the message being sent
...
def constant_load_wait_segment(
    ...
    """
    return ConstantLoadWaitSegment(wait_on=wait_on, cpu_load=cpu_load, mem_load=mem_load)
...
def sampled_load_segment(cpu_load_sampler, freq, duration):
    ...
    Returns:
        ...
        duration.
    """
    return SampledLoadSegment(cpu_load_sampler=cpu_load_sampler, freq=freq, duration=duration)
