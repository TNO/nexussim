...
class NexusSimulationError(NexussimException):
    """Exception raised when a simulation fails."""


def nexussim_simulation_from_yaml(file: Path, max_simulation_time: float = 1.0) -> NexusScene:
...
def nexussim_simulation_from_dict(desc: dict, max_simulation_time: float = 1.0) -> NexusScene:
    """Simulates a Nexus scene from a YAML file.
...
def make_simulation_report(scene: NexusScene):
    ...
    """
    return dict(
        scene_description=scene.as_dict,
...
def _node_kpis(scene: NexusScene):
    ...
    for node_id, compute in scene.computes.items():
        ...
        total_usage = total_usage.with_columns(timestamp=pl.col("timestamp").dt.epoch(time_unit="ns") / 1000000000.0)
        cpu_used_measurements = total_usage["cpu_used"].to_list()
        timestamps = total_usage["timestamp"].to_list()
        kpi_mean_cpu_usage = _mean_cpu_usage(timestamps, cpu_used_measurements)
...
def _mean_cpu_usage(timestamps: ArrayLike, cpu_usage: ArrayLike) -> float:
    ...
    """
    assert len(timestamps) == len(cpu_usage), "Timestamps and CPU usage arrays must have the same length."

    # Not enough data: return a sensible default
    if len(timestamps) == 0 or len(cpu_usage):
        return 0.0

    # If only one timestamp/sample, return that sample (no time to average)
    if len(timestamps) == 1 or len(cpu_usage) == 1:
        return float(np.asarray(cpu_usage).mean())

    return np.dot(np.diff(timestamps), cpu_usage[:-1]) / (timestamps[-1] - timestamps[0])
