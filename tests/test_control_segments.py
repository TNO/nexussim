...
def test_sequence_segment(dynaa_sim, context):
    segment_c = ConstantLoadSegment(cpu_load=3.0, duration=7.0)
    segment_v = SampledLoadSegment(freq=1.0, duration=7.0)
...
def test_while_segment(dynaa_sim, context):
    segment_c = ConstantLoadSegment(cpu_load=2.0, duration=5.0)
    segment_v = SampledLoadSegment(freq=1.0, duration=5.0)
...
def test_concurrent_segment(dynaa_sim, context):
    segment_c = ConstantLoadSegment(cpu_load=2.0, duration=5.0)
    segment_v = SampledLoadSegment(freq=1.0, duration=3.0)
...
def test_choice_segment(dynaa_sim, context):
    ...
    dynaa_sim.run(2)  # runs for 2 seconds
    assert (segment_c.state == SegmentState.RUNNING) != (
        segment_v.state == SegmentState.RUNNING
    )
